...
def parse(args):
    ...
    pred_writer = CoNLLXWriter(word_alphabet, char_alphabet, pos_alphabet, type_alphabet)
    gold_writer = CoNLLXWriter(word_alphabet, char_alphabet, pos_alphabet, type_alphabet)
    pred_filename = os.path.join(result_path, 'pred.txt')  # TODO: Change when training with UDPipe so as not to overwrite the file.
    pred_writer.start(pred_filename)
    gold_filename = os.path.join(result_path, 'gold.txt') # TODO: Change when training with UDPipe so as not to overwrite the file.
    gold_writer.start(gold_filename)
...
if __name__ == '__main__':
    ...
    args_parser.add_argument('--word_embedding', choices=['glove', 'senna', 'sskip', 'polyglot', 'ud'], help='Embedding for words')
