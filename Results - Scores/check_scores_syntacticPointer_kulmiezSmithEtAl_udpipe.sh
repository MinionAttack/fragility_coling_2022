...
for language in {0..5}; do
  for experiment in {1..3}; do
      ...
      Gold_Kulmiez_SmithEtAl_UDPipe/SyntacticPointer/"${treebanks[language]}"/"${treebank_files[language]}"-ud-test.conll \
      Predicted_Kulmiez_SmithEtAl_UDPipe/SyntacticPointer/"${treebanks[language]}"/"${treebank_files[language]}"-iteracion_"$experiment"-predicted-test.conll
      printf "\n"
  done
done
