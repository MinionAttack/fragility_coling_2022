...
for language in {0..6}; do
  for experiment in {1..3}; do
      ...
      Gold_LowResources/SuPar/"${treebanks[language]}"/"${treebank_files[language]}"-ud-test.conll \
      Predicted_LowResources/SuPar/"${treebanks[language]}"/"${treebank_files[language]}"-iteracion_"$experiment"-predicted-test.conll
      printf "\n"
  done
done
