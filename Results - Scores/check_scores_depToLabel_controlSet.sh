...
for language in {0..5}; do
  for experiment in {1..3}; do
      ...
      Gold_ControlSet/DepToLabel/"${treebanks[language]}"/"${treebank_files[language]}"-ud-test.conll \
      Predicted_ControlSet/DepToLabel/"${treebanks[language]}"/"${treebank_files[language]}"-iteracion_"$experiment"-predicted-test.conll
      printf "\n"
  done
done
